...
const BlogEdit: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      ...
      try {
        const blog: Blog = await blogAPI.getBlog(parseInt(id));
        setFormData({
          title: blog.title,
        ...
        setErrors({ non_field_errors: ['Blog not found or you do not have permission to edit it.'] });
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const blog = await blogAPI.updateBlog(parseInt(id), formData);
      navigate(`/blog/${blog.id}`);
    } catch (err: any) {
      if (err.response?.data) {
        setErrors(err.response.data);
      } else {
        setErrors({ non_field_errors: ['Failed to update blog. Please try again.'] });
      }
    } finally {
      setLoading(false);
    }
